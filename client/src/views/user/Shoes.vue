...
export default {
  data() {
    return {
      ...
      DBproducts: [],
      btnsCategories: [
        { text: "Women Shoes", categorie: "women_shoes" },
        { text: "Men shoes", categorie: "men_shoes" },
        { text: "Casual shoes", categorie: "casual_shoes" },
        { text: "Running shoes", categorie: "running_shoes" },
  ...
  methods: {
    ...
    async getProducts() {
      try {
        const res = await this.$store.dispatch("getProductByType", "shoes");
        if (res.status != 200) {
